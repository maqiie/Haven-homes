...
const useSEO = ({ title, description, keywords, ogImage, canonical }) => {
  useEffect(() => {
    ...
    setMeta('og:image:alt',       'HavenRise Homes — Luxury Real Estate in Nairobi', true)
...
const locations = [
  { name:'Westlands',  count:45, img:'https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?w=800&q=80', tall:false },
  { name:'Kilimani',   count:32, img:'https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?w=800&q=80', tall:true  },
  { name:'Lavington',  count:28, img:'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=800&q=80', tall:false },
  { name:'Runda',      count:18, img:'https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=800&q=80', tall:false },
  { name:'Kileleshwa', count:25, img:'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=800&q=80', tall:false },
]
...
const Home = () => {
  ...
  useSEO({
    ...
    description: "Nairobi's premier luxury real estate agency. Browse 500+ exclusive properties in Lavington, Westlands, Kilimani and more. Find your perfect home with HavenRise.",
    keywords:    'luxury real estate Nairobi, properties for sale Nairobi, Westlands apartments, Lavington villas, Kilimani homes, Kenya real estate, HavenRise Homes',
  ...
  const handleFormField = useCallback((key, val) =>
  ...
  const handleForm = useCallback(async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ role:'', name:'', email:'', phone:'', message:'' })
      } else {
        setFormStatus('error')
      }
    } catch {
    ...
    <div style={{ fontFamily:"'DM Sans', sans-serif", background:'#F9F7F4', color:'#1a1a1a' }}>
      ...
      <header style={{ position:'relative', height:'100vh', minHeight:600, overflow:'hidden' }}>
        ...
        */}
        ...
        <div className="hero-content hero-pad" style={{ position:'relative', zIndex:10, height:'100%', display:'flex', flexDirection:'column', justifyContent:'flex-end', padding:'0 6vw 80px' }}>
          ...
          <h1 className="fade-up serif hero-title" style={{ fontSize:'clamp(52px,9vw,110px)', fontWeight:300, color:'#fff', lineHeight:.95, letterSpacing:'-.02em', animationDelay:'.2s' }}>
            HavenRise<br /><em>Homes</em>
          </h1>
      ...
      <section aria-labelledby="about-heading" style={{ background:'#F9F7F4', overflow:'hidden' }}>
        <div className="section-pad" style={{ padding:'80px 6vw 0', maxWidth:1400, margin:'0 auto' }}>
          ...
          <h2 id="about-heading" className="serif" style={{ fontSize:'clamp(44px,6vw,88px)', fontWeight:300, lineHeight:.95, letterSpacing:'-.03em', maxWidth:900 }}>
        ...
        <div className="about-grid section-pad" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', maxWidth:1400, margin:'0 auto', padding:'60px 6vw 0' }}>
          ...
          <div className="about-right" style={{ position:'relative' }}>
            ...
            <div style={{ position:'absolute', top:-16, right:-16, width:48, height:48, borderRadius:'50%', border:'1px solid #8B7355', opacity:.3 }} aria-hidden="true" />
      ...
      <section aria-labelledby="props-heading" className="section-pad" style={{ background:'#F0EDE8', padding:'100px 6vw' }}>
        <div style={{ maxWidth:1400, margin:'0 auto' }}>
          ...
          <div className="prop-grid" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))', gap:24 }}>
            {filtered.map(p => (
              <article key={p.id} className="prop-card" style={{ background:'#fff', overflow:'hidden' }}>
                <div style={{ position:'relative', height:280, overflow:'hidden' }}>
                  <img src={p.image} alt={`${p.title} — ${p.beds} bed property in ${p.location}, Nairobi`}
                    className="prop-img"
                    loading="lazy" decoding="async"
                ...
                <div style={{ padding:'28px 28px 32px' }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    ...
                    <Link to={`/property/${p.id}`} className="ulink"
                      style={{ fontSize:12, letterSpacing:'.12em', textTransform:'uppercase', fontWeight:600, color:'#1a1a1a', textDecoration:'none' }}
      ...
      <section aria-labelledby="gallery-heading" className="section-pad" style={{ padding:'100px 6vw', background:'#1a1a1a' }}>
          ...
          <div className="gal-cols" style={{ columns:'3 260px', gap:10 }}>
            {visibleGal.map((img, i) => (
              <div key={img.id} className="gal-item"
                ...
                style={{ marginBottom:10, breakInside:'avoid', height:galHeights[i % galHeights.length] }}>
                <img src={img.src} alt={`${img.label} — ${img.cat} property in Nairobi`}
        ...
        {lightbox && (
          <div className="lbx" onClick={closeLightbox} role="dialog" aria-modal="true" aria-label={`Lightbox: ${lightbox.label}`}>
            <div style={{ position:'relative', maxWidth:'90vw' }} onClick={e => e.stopPropagation()}>
              <img src={lightbox.src} alt={`${lightbox.label} — full view`}
        ...
        <div className="loc-grid" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gridTemplateRows:'repeat(2,260px)', gap:12 }}>
          {locations.map(loc => (
            <Link key={loc.name}
              ...
              */}
              <img src={loc.img} alt={`${loc.name} neighbourhood — Nairobi real estate`}
                loading="lazy" decoding="async"
      ...
      <section style={{ position:'relative', height:520, overflow:'hidden' }} aria-labelledby="quote-heading">
        <img src="https://images.unsplash.com/photo-1600566753086-00f18fb6b3ea?w=1920&q=80"
      ...
      <section aria-labelledby="testimonials-heading" className="section-pad" style={{ background:'#1a1a1a', padding:'100px 6vw' }}>
        <div style={{ maxWidth:1400, margin:'0 auto' }}>
          <div style={{ marginBottom:60 }}>
            <p style={{ fontSize:11, letterSpacing:'.2em', textTransform:'uppercase', color:'#8B7355', marginBottom:16, fontWeight:600 }}>Client Stories</p>
            <h2 id="testimonials-heading" className="serif" style={{ fontSize:'clamp(34px,4vw,52px)', fontWeight:300, color:'#fff', letterSpacing:'-.02em' }}>Trusted by Thousands</h2>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:24 }}>
            {testimonials.map(t => (
      ...
      <section id="contact" aria-labelledby="contact-heading" className="section-pad" style={{ background:'#F0EDE8', padding:'100px 6vw' }}>
        <div className="contact-grid two-col" style={{ maxWidth:1000, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:80, alignItems:'start' }}>
          <div>
            ...
            <div style={{ width:40, height:1, background:'#8B7355', marginBottom:28 }} aria-hidden="true" />
            <p style={{ fontSize:14, color:'#666', lineHeight:1.8, fontWeight:300, marginBottom:40 }}>
              Whether you're buying, renting, or listing — our expert team is ready to guide you.
            </p>
          ...
          <div style={{ background:'#fff', padding:'48px 44px' }}>
            {formStatus === 'success' ? (
              ...
              <form onSubmit={handleForm} noValidate>
                <fieldset style={{ border:'none', marginBottom:28, padding:0 }}>
                ...
                <div style={{ marginBottom:28 }}>
                  ...
                  <textarea id="contact-message" rows={3} placeholder="Tell us what you're looking for..."
                    value={formData.message}
      ...
      <section className="newsletter-section section-pad" aria-labelledby="newsletter-heading" style={{ position:'relative', padding:'100px 6vw', overflow:'hidden' }}>
        <img src="https://images.unsplash.com/photo-1582407947304-fd86f028f716?w=1920&q=80"
        ...
        <div style={{ position:'relative', zIndex:10, maxWidth:640, margin:'0 auto', textAlign:'center' }}>
          ...
          <form onSubmit={e => e.preventDefault()} className="newsletter-form" style={{ display:'flex', maxWidth:480, margin:'0 auto' }}>
            ...
            <input id="newsletter-email" type="email" placeholder="Your email address" required autoComplete="email"
              style={{ flex:1, padding:'15px 20px', fontSize:14, background:'rgba(255,255,255,.08)', border:'1px solid rgba(255,255,255,.15)', borderRight:'none', color:'#fff', fontFamily:'inherit' }} />
            <button type="submit"
              style={{ padding:'15px 28px', background:'#8B7355', color:'#fff', fontSize:12, letterSpacing:'.15em', textTransform:'uppercase', fontWeight:600, border:'none', cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap' }}>
              Subscribe
